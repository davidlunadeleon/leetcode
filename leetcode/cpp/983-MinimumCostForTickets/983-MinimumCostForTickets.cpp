...
// * Note: This solution is based on the solution presented on the same problem at leetcode.
    ...
    private:
        ...
        int dpCost(vector<int>& days, vector<int>&costs, int index){
            ...
            vector<int> costVector {
                dpCost(days, costs, searchIndex(days, index, days[index] + 1)) + costs[0],
                dpCost(days, costs, searchIndex(days, index, days[index] + 7)) + costs[1],
                dpCost(days, costs, searchIndex(days, index, days[index] + 30)) + costs[2],
            };
            priceMap.insert(pair<int,int>(index, *min_element(costVector.begin(), costVector.end())));
            return priceMap[index];
        }

...
class Solution {
  public:
	vector<string> letterCombinations(string digits) {
		...
		vector<vector<int>> nums{
			...
			vector<int>{'a', 'b', 'c'}, vector<int>{'d', 'e', 'f'},
			vector<int>{'g', 'h', 'i'}, vector<int>{'j', 'k', 'l'},
			vector<int>{'m', 'n', 'o'}, vector<int>{'p', 'q', 'r', 's'},
			vector<int>{'t', 'u', 'v'}, vector<int>{'w', 'x', 'y', 'z'}};
		string empty = "";
  ...
  private:
	void makeCombinations(vector<string> &ans, vector<vector<int>> &numVector,
						  string &digits, int index, string &ansElement) {
...
// * Note: introduce all strings with the format "string" so they can be
// converted to: string
